...
if ($action == 'install') {
    ...
    if (empty($host) || empty($port) || empty($user) || empty($pwd) || empty($database)) {
        ...
        if (empty($errorMsg)) {
            @file_put_contents($databaseFile, '<?php' . PHP_EOL . '//禁止文件直接被访问' . PHP_EOL . 'if (!defined(\'ROOT\')) {' . PHP_EOL . '    header(\'HTTP/1.1 404 Not Found\', true, 404);' . PHP_EOL . '    die();' . PHP_EOL . '}' . PHP_EOL . 'return ' . var_export($mysql, true) . ';' . PHP_EOL);
...
<div class="container"><br>
                ...
                <?php } else { ?>
                    ...
                    <div class="panel-body">
                        ...
                        <ul class="list-group">
                            ...
                            <li class="list-group-item">成功安装后安装文件就会锁定，如需重新安装，请手动删除dg/APP/index目录下database.php配置文件！</li>
                            <?php
    ...
    <footer class="footer">
        <pre><center>Powered by 烟雨寒云 ! </center></pre>
    </footer>
</div>
</body>
</html>